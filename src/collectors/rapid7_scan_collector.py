...
class Rapid7ScanCollector(BaseCollector):
    ...
    async def collect(self, report_type: str = "weekly") -> CollectorResult:
        ...
        try:
            ...
            async with HTTPClient() as client:
                ...
                exposure_data = await self._fetch_asset_vulnerability_exposure(client, base_url, headers)
        ...
        try:
            ...
            while page < max_pages:
                ...
                response = await client.post_raw_response(
                    vuln_url, headers=headers, json_data=request_body, params=params
                )
                ...
                if response.status == 200:
                    ...
                    logger.info("Vulnerabilities endpoint not found (404), using asset-based approach")
            ...
            if len(all_vulns) > 0:
                ...
                await self._enrich_low_count_cves_with_hostnames(client, base_url, headers, cve_asset_map)
    ...
    async def _fetch_all_assets_with_vulns(
        self, client: HTTPClient, assets_url: str, headers: dict[str, str], max_pages: int = 5
    ) -> list[dict[str, Any]]:
        """
        ...
        while page < max_pages:
            ...
            try:
                response = await client.post_raw_response(
                    assets_url, headers=headers, json_data=request_body, params=params
                )

                if response.status == 400:
                    # Filter syntax failed, try with empty body (get ALL assets)
                    logger.warning(
                        "Vulnerability filter failed (status 400), trying to fetch ALL assets without filter"
                    )
                    request_body = {}
                    response = await client.post_raw_response(
                        assets_url, headers=headers, json_data=request_body, params=params
                ...
                if response.status == 200:
                    ...
                    response = await client.post_raw_response(assets_url, headers=headers, json_data={}, params=params)
    ...
    def _build_cve_map_from_vulnerability_findings(self, vulns: list[dict[str, Any]]) -> dict[str, Any]:
        """
    ...
    async def _enrich_low_count_cves_with_hostnames(
        self, client: HTTPClient, base_url: str, headers: dict[str, str], cve_exposure: dict[str, Any]
    ):
        """
        ...
        logger.info("Note: Cloud API doesn't support CVE-based asset filtering, using brute-force approach")
        ...
        try:
            ...
            for page in range(max_pages):
                ...
                all_found = all(
                    len(assets) >= cves_to_enrich.get(cve_id, 999) for cve_id, assets in cve_to_assets.items()
                )
    ...
    def _build_cve_map_from_assets(self, assets: list[dict[str, Any]]) -> dict[str, Any]:
        """
        ...
        if assets and len(assets) > 0:
            ...
            logger.info(
                f"Sample asset 'new' field type: {type(new_vulns)}, length: {len(new_vulns) if isinstance(new_vulns, list) else 'N/A'}"
            )
            logger.info(
                f"Sample asset 'remediated' field type: {type(remediated_vulns)}, length: {len(remediated_vulns) if isinstance(remediated_vulns, list) else 'N/A'}"
            )
        ...
        for idx, asset in enumerate(assets):
            ...
            if idx < 5:  # Log first 5 for debugging
                logger.info(
                    f"Asset {asset_name}: {len(active_vulns)} active vulns from 'new' field (total_vulnerabilities={total_vulns})"
                )
                if idx == 0 and len(active_vulns) > 0:
                    logger.info(
                        f"  First vuln keys: {list(active_vulns[0].keys()) if isinstance(active_vulns[0], dict) else type(active_vulns[0])}"
                    )
            ...
            for vuln in active_vulns:
                ...
                for cve_id in cve_ids:
                    ...
                    if asset_id not in cve_info["asset_ids"]:
                        ...
                        os_name = asset.get("os_name", "").lower()
                        asset.get("os_system_name", "").lower()
                        tags = asset.get("tags", [])
                        ...
                        is_azure = (
                            "azure" in asset_name.lower() or "azurevm" in asset_name.lower() or "azure" in tag_str
                        )
                        ...
                        if "database" in os_name or "sql" in os_name or "mysql" in os_name:
                            ...
                            "server" in asset_type.lower()
                            or "linux" in asset_type.lower()
                            or "unix" in asset_type.lower()
                        ):
    ...
    def _format_cve_exposure_summary(
        self, cve_asset_map: dict[str, dict[str, Any]], total_items: int
    ) -> dict[str, Any]:
        """
