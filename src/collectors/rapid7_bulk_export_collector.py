...
class Rapid7BulkExportCollector(BaseCollector):
    ...
    async def collect(self, report_type: str = "weekly") -> CollectorResult:
        ...
        try:
            ...
            if cached_data:
                ...
                logger.info("  No API fetch needed - data is < 24 hours old")
                return CollectorResult(
                    source=self.source_name, success=True, data=[cached_data], record_count=record_count
                )
            else:
        ...
        if storage_account_name and storage_account_key:
            try:
                ...
                if cached_data:
                    ...
                    logger.info("  Cache is fresh and reports will complete instantly")
                    return CollectorResult(
                        source=self.source_name, success=True, data=[cached_data], record_count=record_count
                    )
                else:
        ...
        try:
            ...
            async with HTTPClient() as client:
                ...
                if not export_id:
                    ...
                    return CollectorResult(
                        source=self.source_name, success=False, error="Failed to create export", record_count=0
                    )
                ...
                if not download_urls:
                    ...
                    return CollectorResult(
                        source=self.source_name, success=False, error="Export timeout or no data", record_count=0
                    )
    ...
    async def _create_vulnerability_export(self, client: HTTPClient, graphql_url: str, headers: dict[str, str]) -> str:
        """
        ...
        try:
            response = await client.post_raw_response(graphql_url, headers=headers, json_data=payload)
    ...
    async def _poll_export_status(
        ...
        for attempt in range(max_attempts):
            payload = {"query": query, "variables": {"id": export_id}}
    ...
    async def _download_and_parse_exports(
        self, client: HTTPClient, download_urls: list[dict[str, str]], headers: dict[str, str]
    ) -> dict[str, Any]:
        """
    ...
    def _parse_vulnerability_dataframe(self, df, cve_data: dict[str, Any], asset_data: dict[str, Any]):
        """
    ...
    def _build_cve_exposure_from_export_data(
        self, cve_data: dict[str, Any], asset_data: dict[str, Any]
    ) -> dict[str, Any]:
        """
