...
def run(input: GateInput, llm_client: Any, report_type: str) -> GateResult:
    ...
    if report_type.upper() != "QUARTERLY":
        ...
        return GateResult(gate_id="1F", status="COMPLETE", payload={"skipped": "Not a quarterly report"})

    # Get report from Gate 5
    g5 = input.prior_results.get("5")
    if not g5 or g5.status != "COMPLETE":
        return GateResult(gate_id="1F", status="HALT", halt_reason="Gate 5 (Report Draft) did not complete", payload={})

    report = g5.payload.get("report", {})

    logger.info("=" * 80)
    logger.info("SOURCE AUDIT - COMPREHENSIVE VERIFICATION")
    logger.info("This audit shows exactly where every source is used.")
    logger.info("=" * 80)
    ...
    for idx, incident in enumerate(incidents, 1):
        ...
        for term in FORBIDDEN_TERMS:
            ...
            logger.info("   ✓ Uses actual company name")
    ...
    if not osint_sources:
        ...
        for source in osint_sources:
            ...
            if not url or url == "No URL":
                ...
                logger.error("   ❌ HALLUCINATED URL - news.illumina.com domain doesn't exist")
            else:
            ...
            if citation_num in all_citations:
                if citation_num in exec_citations:
                    logger.info("   ✓ Cited in Executive Summary")
                if citation_num in geo_citations:
                    logger.info("   ✓ Cited in Geopolitical Relevance")
            else:
                osint_issues.append(f"⚠️  [{citation_num}] {title}: Listed but never cited")
                logger.warning("   ⚠️  LISTED BUT NEVER CITED IN REPORT")
