...
def _validate_weekly_statistics(gate_input: GateInput) -> GateResult:
    ...
    if has_tier1_data and cve_count == 0:
        warnings.append(
            f"Tier 1 sources returned data but 0 CVEs extracted. "
            f"This may indicate data parsing issues."
        )
    validations.append({
    ...
    total_tier1_records = sum(
        src.records_returned 
        for src in tier1_sources
    )
    ...
    if osint_article_count == 0:
        warnings.append(
            "0 OSINT articles collected. OSINT corroboration and open signals will be limited."
        )
    validations.append({
    ...
    logger.info(f"Weekly statistics validation: {len(validations)} checks, "
...
def _validate_quarterly_statistics(gate_input: GateInput) -> GateResult:
    ...
    if intel471_records == 0:
        warnings.append(
            "0 Intel471 records collected. Quarterly strategic analysis will lack breach intelligence."
        )
    validations.append({
    ...
    if crowdstrike_records == 0:
        warnings.append(
            "0 CrowdStrike records collected. Quarterly report will lack threat actor intelligence."
        )
    validations.append({
    ...
    if not has_geopolitical:
        warnings.append(
            "No geopolitical context signals found. Quarterly strategic analysis may lack depth."
        )
    validations.append({
