...
def run(input: GateInput, llm_client: Any, report_type: str) -> GateResult:
    ...
    if report_type.upper() == "QUARTERLY":
        ...
        if breach_landscape:
            ...
            if stat_cards:
                for card in stat_cards:
                    ...
                    if change_pct and not (
                        change_pct.startswith("+") or change_pct.startswith("-") or change_pct == "0%"
                    ):
        ...
        if statistics:
            ...
            if cve_count_report != cve_count_actual:
                issues.append(
                    f"CVE count mismatch: statistics={cve_count_report}, actual CVEs in report={cve_count_actual}"
                )
            else:
        ...
        if crowdstrike_data:
            ...
            if crowdstrike_detections:
                ...
                if not detection_mentioned:
                    warnings.append(
                        f"CrowdStrike detections collected ({len(crowdstrike_detections)}) but none specifically referenced"
                    )
    ...
    return GateResult(gate_id="1D", status=status, halt_reason=halt_reason, payload=summary)
