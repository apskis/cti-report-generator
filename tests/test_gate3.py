...
from gates.models import GateInput, GateResult, IOC, SourceRecord
...
def test_unattributed_when_no_source_names_actor():
    ...
    gi = GateInput(
        report_type="WEEKLY", period_start="a", period_end="b",
...
def test_actor_attribution_from_intel471():
    ...
    gi = GateInput(
        report_type="WEEKLY", period_start="a", period_end="b",
        tier1_data={"Intel471": [
            {"indicator": "1.2.3.4", "actor": "BadGroup", "campaign": "OpFoo", "confidence": "high"}
        ]},
        prior_results={"1": g1, "2": g2},
    )
    r = run(gi, _FakeLLM(), "WEEKLY")
...
def test_region_only_populated_for_quarterly():
    ...
    g2 = GateResult("2", "COMPLETE", {"iocs": iocs})
    gi = GateInput(
        report_type="QUARTERLY", period_start="a", period_end="b",
        tier1_data={"Intel471": [
            {"indicator": "1.2.3.4", "actor": "BadGroup", "region": "RU"}
        ]},
        prior_results={"1": g1, "2": g2},
    )
