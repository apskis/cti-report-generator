...
def _rec(name: str, status: str, enabled: bool = True) -> SourceRecord:
    return SourceRecord(
        source_name=name, tier=1, records_returned=0 if "GAP" in status else 1,
        period_start="2026-05-12", period_end="2026-05-19", status=status, enabled=enabled,
    )


def test_halt_raises_when_two_tier1_sources_have_gap():
    records = [
    ...
    assert "2" in exc.value.reason or "Tier 1" in exc.value.reason
